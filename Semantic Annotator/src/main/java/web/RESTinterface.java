...
public class RESTinterface {
	...
	public Response postResource(@Context UriInfo uriinfo, Iterable<Node[]> input, @Context HttpHeaders headers) {
		...
		// Considering slugs
		String newlyCreatedResourceLocalName = headers.getRequestHeader("Slug") == null ? null
				: Normalizer.normalize(headers.getRequestHeader("Slug").get(0), Form.NFD).replaceAll("[^ -~]", "");
		...
		*/
		...
		try {
			return Response.created( new URI("http://example.org") ).build();
		} catch (URISyntaxException e) {
